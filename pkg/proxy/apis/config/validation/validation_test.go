	...
	for name, testCase := range map[string]struct {
		...
		"dual-stack": {
			mutateConfigFunc: func(config *kubeproxyconfig.KubeProxyConfiguration) {
				...
				config.DetectLocal.ClusterCIDRs = []string{"192.168.59.0/24", "fd00:192:168:59::/64"}
			},
		},
		...
		"alternate metrics port": {
			mutateConfigFunc: func(config *kubeproxyconfig.KubeProxyConfiguration) {
				config.MetricsBindPort = 54321
			},
		},
		...
		"ClusterCIDR is wrong IP family": {
			mutateConfigFunc: func(config *kubeproxyconfig.KubeProxyConfiguration) {
				config.DetectLocal.ClusterCIDRs = []string{"fd00:192:168::/64"}
			},
		},
		...
		"SyncPeriod must be > 0": {
			...
			expectedErrs: field.ErrorList{
				field.Invalid(newPath.Child("SyncPeriod"), metav1.Duration{Duration: -5 * time.Second}, "must be greater than 0"),
				field.Invalid(newPath.Child("SyncPeriod"), metav1.Duration{Duration: 3 * time.Second}, "must be greater than or equal to KubeProxyConfiguration.MinSyncPeriod"),
			},
		},
		"MinSyncPeriod must be >= 0": {
			...
			expectedErrs: field.ErrorList{field.Invalid(newPath.Child("MinSyncPeriod"), metav1.Duration{Duration: -3 * time.Second}, "must be greater than or equal to 0")},
		},

...
func deleteStaleServiceConntrackEntries(ct Interface, ipFamily v1.IPFamily, svcPortMap proxy.ServicePortMap, serviceUpdateResult proxy.UpdateServiceMapResult, endpointsUpdateResult proxy.UpdateEndpointsMapResult) {
	...
	if err := ct.ClearEntries(getUnixIPFamily(ipFamily), filters...); err != nil {
...
func getUnixIPFamily(ipFamily v1.IPFamily) uint8 {
...
// filterForIP returns *conntrackFilter to delete the conntrack entries for connections specified
// by the destination IP(original direction).
...
// filterForPort returns *conntrackFilter to delete the conntrack entries for connections specified
// by the destination Port(original direction) and IPFamily.
...
// filterForNAT returns *conntrackFilter to delete the conntrack entries for connections specified
// by the destination IP(original direction) and source IP(reply direction).
...
// filterForPortNAT returns *conntrackFilter to delete the conntrack entries for connections specified
// by the destination Port(original direction) and source IP(reply direction).

...
func (s *ProxyServer) Run() error {
	...
	if utilfeature.DefaultFeatureGate.Enabled(features.MultiCIDRServiceAllocator) {
		serviceCIDRConfig, err := config.NewServiceCIDRConfig(informerFactory.Networking().V1alpha1().ServiceCIDRs(), s.Config.ConfigSyncPeriod.Duration)
		if err != nil {
			s.logger.Error(err, "failed to create serviceCIDR config for watches")
			return err
		}

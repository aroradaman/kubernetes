...
func TestValidateKubeProxyConntrackConfiguration(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"invalid TCPBeLiberal < 0": {
			...
			expectedErrs: field.ErrorList{field.Invalid(newPath.Child("KubeConntrackConfiguration.TCPBeLiberal"), -1, "must be greater than or equal to 0")},
		},

...
type KubeProxyLinuxConfiguration struct {
	...
	// masqueradeAll tells kube-proxy to SNAT all traffic sent to Service cluster IPs, when using the
	// linux proxy modes (iptables, ipvs and nftables). This may be required with some CNI plugins.
	MasqueradeAll bool
}

// KubeProxyWindowsConfiguration contains windows-platform-related configuration details for
...
type KubeProxyConfiguration struct {
	...
	// primary IPs are. Note that the name is a historical artifact, and kube-proxy does
	// not actually bind any sockets to these IPs.
	NodeIPOverride []string
	// healthzBindAddresses is a list of CIDR ranges that contains a valid node IP on which
	// health check server will be served on, defaulting to [ "0.0.0.0/0", "::/0" ].

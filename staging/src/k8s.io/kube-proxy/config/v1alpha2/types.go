...
type KubeProxyLinuxConfiguration struct {
	...
	MasqueradeAll bool `json:"masqueradeBit"`
}
...
type KubeProxyConfiguration struct {
	...
	// primary IPs are.
	NodeIPOverride []string `json:"nodeIPOverride"`
	...
	HealthzBindAddresses []string `json:"healthzBindAddresses"`
	// healthzBindPort is the port on which helathz server will be exposed, defaulting to 10256.
	HealthzBindPort int32 `json:"healthzBindPort"`
	...
	Linux KubeProxyLinuxConfiguration `json:"linux"`
	// windows contains windows-related configuration options.
	Windows KubeProxyWindowsConfiguration `json:"windows"`

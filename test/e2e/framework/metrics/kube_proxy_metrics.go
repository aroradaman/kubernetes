...
// GetMetricValuesForLabel returns true if all metrics are the same as the arguments.
func (m *KubeProxyMetrics) GetMetricValuesForLabel(metricName, label string) map[string]int64 {
	return testutil.GetMetricValuesForLabel(testutil.Metrics(*m), metricName, label)
}

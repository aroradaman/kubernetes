...
func Validate(config *kubeproxyconfig.KubeProxyConfiguration) field.ErrorList {
	...
	allErrs = append(allErrs, validateKubeProxyConntrackConfiguration(config.Linux.Conntrack, newPath.Child("KubeProxyConntrackConfiguration"))...)
	...
	for i, addr := range config.NodeIPOverride {
		if netutils.ParseIPSloppy(addr) == nil {
			allErrs = append(allErrs, field.Invalid(newPath.Child("NodeIPOverride").Index(i), addr, "not a valid textual representation of an IP address"))
		}
	}
...
func validateDualStackCIDRStrings(cidrs []string, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	switch {
	case len(cidrs) == 0:
		allErrs = append(allErrs, field.Invalid(fldPath, cidrs, "must contain at least one CIDR"))
	case len(cidrs) > 2:
		allErrs = append(allErrs, field.Invalid(fldPath, cidrs, "must be a valid CIDR list (e.g. [10.100.0.0/16, fde4:8dba:82e1::/48]"))
	default:
		for i, cidr := range cidrs {
			if _, _, err := netutils.ParseCIDRSloppy(cidr); err != nil {
				allErrs = append(allErrs, field.Invalid(fldPath.Index(i), cidr, "must be a valid CIDR block (e.g. 10.100.0.0/16 or fde4:8dba:82e1::/48)"))
			}
		}
		if len(cidrs) == 2 {
			ifDualStack, err := netutils.IsDualStackCIDRStrings(cidrs)
			if err == nil && !ifDualStack {
				allErrs = append(allErrs, field.Invalid(fldPath, cidrs, "must contain at least one CIDR per IP family"))
			}
		}
	}
	return allErrs

...
func decode(msg []byte, strict bool) (*Counter, error) {
	...
	for reader.Len() > 0 {
		...
		// we read the initial 2 bytes of the binary string directly into the 'length' variable.
		if err := binary.Read(reader, binary.NativeEndian, &length); err != nil {
			return nil, err
		}

		// STEP 2. parse type   [2 bytes]
		// we read the next 2 bytes of the binary string directly into the 'attrType' variable.
		...
		if length%4 != 0 {
			padding := int64((length/4+1)*4 - length)
			if _, err := reader.Seek(padding, io.SeekCurrent); err != nil {
				return nil, err
			}
		}
	}

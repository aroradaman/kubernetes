...
func tweakPodCIDRs(podCIDRs ...string) nodeTweak {
	return func(n *v1.Node) {
		for _, podCIDR := range podCIDRs {
			n.Spec.PodCIDRs = append(n.Spec.PodCIDRs, podCIDR)
		}
	}
}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				...
				require.Equal(t, nodeManager.NodeIPs(), tc.expectedNodeIPs)
				require.Equal(t, nodeManager.PodCIDRs(), tc.expectedPodCIDRs)
			}
		})
	}
}
...
func TestNodeManagerNode(t *testing.T) {
	makeNode := func(resourceVersion string) *v1.Node {
		return &v1.Node{
			ObjectMeta: metav1.ObjectMeta{
				Name:            testNodeName,

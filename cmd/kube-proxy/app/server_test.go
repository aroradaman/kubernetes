...
func Test_detectNodeIPs(t *testing.T) {
	cases := []struct {
		...
		{
			name:           "NodeIPOverride is single-stack IPv4 unicast address and no Node object",
			rawNodeIPs:     nil,
		...
		{
			name:           "NodeIPOverride is single-stack IPv6 unicast address and no Node object",
			rawNodeIPs:     nil,
		...
		{
			name:           "NodeIPOverride is single-stack IPv4 unspecified and node with IPv4 InternalIP set",
		...
		{
			name:           "NodeIPOverride is single-stack IPv6 unspecified and node with IPv4 InternalIP set",
			rawNodeIPs:     []net.IP{netutils.ParseIPSloppy("192.168.1.1")},
			nodeIPOverride: []string{"::"},
			expectedFamily: v1.IPv4Protocol,
			expectedIPv4:   "192.168.1.1",
		...
		{
			name:           "NodeIPOverride is single-stack IPv4 unspecified and node with IPv6 InternalIP set",
		...
		{
			name:           "NodeIPOverride is single-stack IPv6 unspecified and node with IPv6 InternalIP set",
			rawNodeIPs:     []net.IP{netutils.ParseIPSloppy("fd00:1234::1")},
			nodeIPOverride: []string{"::"},
			expectedFamily: v1.IPv6Protocol,
			expectedIPv4:   "127.0.0.1",
		...
		},
		{
			...
			nodeIPOverride: []string{"127.0.0.1", "::1"},
			expectedFamily: v1.IPv4Protocol,
			expectedIPv4:   "127.0.0.1",
			expectedIPv6:   "::1",
		},
		{
			name: "Dual stack, override with unspecified IPv4",
			rawNodeIPs: []net.IP{
				netutils.ParseIPSloppy("2001:db8::2"),
				netutils.ParseIPSloppy("90.90.90.90"),
			},
		...
		},
		{
			...
			nodeIPOverride: []string{"::1", "127.0.0.1"},
			expectedFamily: v1.IPv6Protocol,
			expectedIPv4:   "127.0.0.1",
			expectedIPv6:   "::1",
		},
		{
			name: "Dual stack, override with unspecified IPv6",
			rawNodeIPs: []net.IP{
				netutils.ParseIPSloppy("90.90.90.90"),
				netutils.ParseIPSloppy("2001:db8::2"),
			},
...
func Test_checkBadConfig(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			proxy: &ProxyServer{
				Config: &kubeproxyconfig.KubeProxyConfiguration{
					DetectLocal:       kubeproxyconfig.DetectLocalConfiguration{ClusterCIDRs: []string{"10.0.0.0/8"}},
		...
		},
		{
			...
			proxy: &ProxyServer{
				Config: &kubeproxyconfig.KubeProxyConfiguration{
					DetectLocal:       kubeproxyconfig.DetectLocalConfiguration{ClusterCIDRs: []string{"10.0.0.0/8", "fd09::/64"}},
		...
		},
		{
			...
			proxy: &ProxyServer{
				Config: &kubeproxyconfig.KubeProxyConfiguration{
					DetectLocal:       kubeproxyconfig.DetectLocalConfiguration{ClusterCIDRs: []string{"fd09::/64"}},
					NodePortAddresses: []string{"192.168.0.0/24"},
				},
...
func Test_checkBadIPConfig(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			proxy: &ProxyServer{
				Config: &kubeproxyconfig.KubeProxyConfiguration{
					DetectLocal: kubeproxyconfig.DetectLocalConfiguration{ClusterCIDRs: []string{"10.0.0.0/8", "fd01:2345::/64"}},
				},
				PrimaryIPFamily: v1.IPv4Protocol,
			},
			ssErr: false,
		...
		},
		{
			...
			proxy: &ProxyServer{
				Config: &kubeproxyconfig.KubeProxyConfiguration{
					DetectLocal: kubeproxyconfig.DetectLocalConfiguration{ClusterCIDRs: []string{"fd01:2345::/64", "10.0.0.0/8"}},
				},
				PrimaryIPFamily: v1.IPv4Protocol,
			},
			ssErr: false,
		...
		},
		{
			...
			proxy: &ProxyServer{
				Config: &kubeproxyconfig.KubeProxyConfiguration{
					DetectLocal: kubeproxyconfig.DetectLocalConfiguration{ClusterCIDRs: []string{"fd01:2345::/64"}},
				},
				PrimaryIPFamily: v1.IPv4Protocol,
			},
			ssErr:   true,
